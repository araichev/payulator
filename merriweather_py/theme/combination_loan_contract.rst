...
.. list-table::
    ...
    * - **Payment Schedule**
      - {{ num_payments_interest_only }} consecutive monthly payments of {{ "{:,.2f}$".format(periodic_payment.interest_only) }} beginning on {{ first_payment_date.interest_only }} and ending on {{ last_payment_date.interest_only }}, which covers the interest-only portion of the loan.
